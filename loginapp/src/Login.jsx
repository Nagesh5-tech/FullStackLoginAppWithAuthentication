...
function Login({ setUser, onNewUser }){
    ...
    const [wakingStatus, setWakingStatus]=useState(""); // e.g. "Waking server... (attempt 2 of 5)"

    // Pre-wake backend when login page loads so it's ready by the time user clicks
    ...
    const handleSubmit = async(e) => {
        ...
        try{
            const { ok, status, data } = await apiLogin(username, password, (attempt, max) => {
